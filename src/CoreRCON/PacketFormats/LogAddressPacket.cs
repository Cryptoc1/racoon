...
public readonly record struct LogAddressPacket(string Body, bool HasPassword, DateTime Timestamp)
{
    ...
    private static readonly string[] DateFormats = ["MM/dd/yyyy - HH:mm:ss", "MM/dd/yyyy - HH:mm:ss.fff"];
    ...
    public static bool TryFromBytes(in byte[] buffer, out LogAddressPacket packet)
    {
        ...
        static bool ContainsMarker(in byte[] buffer) => buffer[0] is 0xFF && buffer[1] is 0xFF && buffer[2] is 0xFF && buffer[3] is 0xFF;
    }
}
