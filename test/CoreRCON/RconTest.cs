...
[TestClass]
public sealed class RconTest
{

    private RCON rconClient = default!;
    ...
    {
        rconClient = new RCON(_ip, _port, _password, new(TimeSpan.FromSeconds(1), false));
        await rconClient.ConnectAsync();
    }


    [TestMethod]
    [ExpectedException(typeof(RCONAuthenticationException))]
    ...
    {
        rconClient.Dispose();
        rconClient = new RCON(_ip, _port, _password, new(true)); //Enable multi packetsupport
        await rconClient.ConnectAsync();
    ...
    {
        rconClient.Dispose();
        rconClient = new RCON(_ip, _port, _password, new(TimeSpan.Zero, false));
        await rconClient.ConnectAsync();
